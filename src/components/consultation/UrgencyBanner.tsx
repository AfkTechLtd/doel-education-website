...
const stats = [
  ...
  {
    icon: CalendarClock,
    label: "Next Fall intake deadline: Sep 1",
    highlight: false,
  },
];
    ...
    <div className="bg-secondary/10 border-y border-secondary/30">
      <div className="max-w-7xl mx-auto px-6 py-3.5 flex flex-wrap items-center justify-center gap-6">
        {stats.map(({ icon: Icon, label, highlight }) => (
          <span
            key={label}
            className="flex items-center gap-2 text-sm font-inter font-medium text-slate-700"
          >
            <Icon
        ...
        <span className="text-sm font-semibold text-primary font-inter">
          Spots fill fast. Book now, it&apos;s free.
        </span>
      </div>
    </div>
  );
